 ...
 *
 * Tree-sitter's grammar inheritance mechanism does not automatically propagate
 * external scanners from parent grammars. When extending a grammar that uses a
 * scanner, the child grammar's parser expects scanner functions namespaced to
 * the child (e.g., tree_sitter_[child]_external_scanner_*), but these functions
 * do not exist unless explicitly provided.
 *
 * This limitation is a known issue in tree-sitter's architecture:
 * - Issue #645 (2020): https://github.com/tree-sitter/tree-sitter/issues/645
 * - Discussion #1915 (2023): https://github.com/tree-sitter/tree-sitter/discussions/1915
 *
 * The problem stems from tree-sitter's simplified build model: grammars are
 * compiled from just two files (parser.c and scanner.c) with no dependency
 * linking mechanism. There is no way for the build system to link the child
 * parser to the parent's scanner functions.
 *
 * WORKAROUND:
 *
 ...
 *
 * This approach allows the child grammar to handle the same external tokens
 * (AUTOMATIC_SEMICOLON, TEMPLATE_CHARS, TERNARY_QMARK, etc.) that the parent
 * grammar requires, while maintaining independent compilation.
 *
 * MAINTENANCE:
 *
 * When updating the [parent grammar] dependency, this scanner may need to be
...
#include "tree_sitter/parser.h"
 ...
 void tree_sitter_objj_external_scanner_destroy(void *p) {}

 unsigned tree_sitter_objj_external_scanner_serialize(void *payload, char *buffer) { return 0; }

 void tree_sitter_objj_external_scanner_deserialize(void *p, const char *b, unsigned n) {}
 ...
 bool tree_sitter_objj_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
